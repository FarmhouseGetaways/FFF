-- Paid moves from the catalog to the day (16 Sep 2026, Cory: "Move the paid
-- button from vendor to the numbers ... vendor only enter once to pull the
-- info to the numbers").
--
-- Ordering is something that happens on a date, from a vendor, for a price -
-- not a property of the product. So how many were ordered and whether that
-- order has been paid for both belong on the count line, which already keeps
-- its own vendor/price/cost snapshot. Same item, different vendor on a
-- different day, is then just two lines.
alter table daily_count_lines add column if not exists paid boolean not null default false;

alter table products drop column if exists ordered_qty;
alter table products drop column if exists paid;
...
begin
  ...
  insert into daily_count_lines
    (closeout_id, entity_id, product_id, product_name, vendor, ordered, sold, price, cost, paid, sort_order)
  select v_closeout.id, p_entity, l.product_id, l.product_name, l.vendor,
         ...
         coalesce(l.paid, false), coalesce(l.sort_order, 0)
    from jsonb_to_recordset(coalesce(p_lines, '[]'::jsonb)) as l(
      product_id uuid, product_name text, vendor text,
      ordered numeric, sold numeric, price numeric, cost numeric,
      paid boolean, sort_order int
    );
